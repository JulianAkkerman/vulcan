...
# A table of string values (python type: List[List[str]]). This is also the format to use for tagged sentences.
FORMAT_NAME_STRING_TABLE = "string_table"

# A table of objects (python type: List[List[Tuple[str, Any]]]). In the tuples, the first entry is again a format name
#  from this file, and the second entry is an object of that format. This is also the format to use for tagged
#  sentences, if the tags are not just strings.
